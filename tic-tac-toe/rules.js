...
function begin_play(){
	...
	if(isEmpty(document.getElementById("player1_id").value) || isEmpty(document.getElementById("player2_id").value)){
		alert("Please enter both names in order to play!");
	}else{
...
function reset_play(){
	...
	//change flavor text (NOT MENTIONED IN REQUIREMENTS)
...
//A helper function to validate the state of the game.
function logicCheck(board){
	if(
		...
		return board[6];
	}else if( //all games with middle part of winning combo
...
function play() {
	...
	if(!started){
		alert("Please start the game before playing!")
		return;
	}

	if(table_ids.includes(movebox.value)){
		let turn_mark = 'X';
		let index = -1;
		if(turn == 0){
			//Setup char correctly
			turn_mark = 'O';
		}
		switch(movebox.value){
			case 'A1':
				index = 0;
				break;
			case 'A2':
		...
		if(board_state[index] != -1){
			//Invalid move
			alert("This is an invalid move! Please try again in an open spot.");
		}else{
			//Valid move
			...
			document.getElementById(movebox.value).innerHTML = turn_mark;

			//check for game win
			if(logicCheck(board_state) != -1){
				if(logicCheck(board_state) == 0){
					alert("Player 2 wins!");
				}else{
					alert("Player 1 wins!");
				}
